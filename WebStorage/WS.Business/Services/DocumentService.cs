...
namespace WS.Business.Services
{
    public class DocumentService
    {
        ...
        {
            Document doc = new Document {Id=(userId+file.FileName+parentId).GetHashCode(), IsFile = true, Size = (int)file.Length, Name = file.FileName,Extention=file.ContentType , UserId=userId, ParentId = parentId };
            repo.Create(doc);
        }
        public void Create(string folder, string userId, int parentId=0)
        {
            Document doc = new Document { Id = (userId + folder + parentId).GetHashCode(), IsFile = false, Size = 0, Name = folder, Extention = "Folder", UserId = userId, ParentId=parentId };
            repo.Create(doc);
        }
